...
import gen_tileset_eimap
from gen_tileset_eimap import test_err, convert_tileset
...
def convert_texture(img_name):
    ...
    subprocess.check_call([
        os.path.join(gen_tileset_eimap.TOOLS_DIR, "nvcompress.exe"),
        "-color", "-bc1", "-silent",
    ...
    print("imgname_",img_name[:-3])
    with open(os.path.join(OUT_DIR, img_name[:-3] + "_res", img_name + ".mmp"), "wb") as f:
...
def sanitize_mod(mod_def):
    ...
    for i, tileset in enumerate(mod_def["maps"]):
        ...
        mod_def["maps"][i] = os.path.join(gen_tileset_eimap.MAPS_DIR, tileset + ".yaml")

    mod_def["date"] = datetime.datetime.today().strftime("%d.%m.%Y")

def create_mod(tileset):
    ...
    print("tset:",map_def["map_name"])
    #print("tset_:",tileset[5:-5])
    if not os.path.isdir(os.path.join(OUT_DIR, map_def["map_name"]+"_res")):
        os.makedirs(os.path.join(OUT_DIR, map_def["map_name"]+"_res"))

    with ThreadPoolExecutor(max(2, os.cpu_count() // 2)) as executor:
        print("Converting tilesets")
        futures = [executor.submit(convert_tileset, tileset)]
...
#    mod_def["version"]))
    ...
    #subprocess.call(os.path.join(gen_tileset_eimap.TOOLS_DIR, "eipacker.exe") +
    ...
    with open(tileset) as f:
        map_def = yaml.safe_load(f)
    subprocess.call(os.path.join(gen_tileset_eimap.TOOLS_DIR, "eipacker.exe") +
        " " + os.path.join(OUT_DIR, map_def["map_name"]+"_res"), shell=True)

    time.sleep(1) # Fix error with zipfile
    shutil.copy(os.path.join(OUT_DIR, map_def["map_name"]+".res"),os.path.join(MOD_OUT_DIR))
    ...
    #    z.write(os.path.join(OUT_DIR, map_def["map_name"]+".res"), arcname=os.path.join(mod_def["mod_name"], map_def["map_name"]+".res"))
...
if __name__ == "__main__":
    ...
    gen_tileset_eimap.read_config()

    MOD_OUT_DIR = gen_tileset_eimap.OUT_DIR
    ...
    gen_tileset_eimap.OUT_DIR = temp_dir.name
